...
import time
import pulp
from utils import flatten, Action, ForestActionCandidates
# np.set_printoptions(suppress=True, precision=3)


class ForestActionExtractor():
    def __init__(self, mdl, X, theta=0.5, Y=[],
        ...
        self.AC_ = ForestActionCandidates(X, mdl, Y=Y, feature_names=feature_names, feature_types=feature_types, feature_categories=feature_categories, feature_constraints=feature_constraints, max_candidates=max_cancidates, tol=tol)
        self.L_ = self.AC_.L_
        self.H_ = self.AC_.H_
        self.tol_ = tol
    ...
    def extract(self, x, 
                W=[], max_change_num=4, cost_type='uniform', use_threshold=True, alpha=0.0, n_neighbors=10, p_neighbors=2, subsample=20, 
                solver='CPLEX_PY', time_limit=300, log_stream=False, mdl_name='', log_name='', init_sols={}, verbose=False):
        self.x_ = x
        ...
        self.A_, self.C_, self.I_ = self.AC_.generateActions(x, self.y_, cost_type=cost_type, use_threshold=use_threshold)
...
def _check_forest_ce(N=1):
    ...
    print()
    print('# Demonstration: linear_ce.py')
    print('- Dataset: German Dataset')
    print('- Classifier: Random Forest Classifier')
    print()
    for n,x in enumerate(X_undesirable[:N]):
        print('# {}{} Individual with high risk of default'.format(n+1, 'st' if (n+1)%10==1 else ('nd' if (n+1)%10==2 else ('rd' if (n+1)%10==3 else  'th'))))
        a = ce.extract(x, max_change_num=3, cost_type='DACE', alpha=0.01, use_threshold=True)
        if(a!=-1): print(a)
