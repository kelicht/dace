...
class LinearActionExtractor():
    ...
    def extract(self, x, 
        ...
        xi  = [pulp.LpVariable('xi_{}'.format(d), cat='Continuous', lowBound=self.x_[d]+self.lb_[d], upBound=self.x_[d]+self.ub_[d]) for d in range(self.D_)]
        ...
        for d in range(self.D_): prob += xi[d] - act[d] == self.x_[d]
...
def _check_linear_ce(N=1):
    import pandas as pd
    from sklearn.linear_model import LogisticRegression
    from utils import DatasetHelper
    np.random.seed(1)

    D = DatasetHelper(dataset='h', feature_prefix_index=False)
    ...
    print('- Dataset: FICO HELOC Dataset')
    print('- Classifier: l2-Regularized Logistic Regression Classifier')
    print()
